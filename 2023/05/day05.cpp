...
vector<vector<long>> Parse(string sourceToDestination) {
  ...
  for (int i = 1; i < lines.size(); i++) {
    ...
    for (string num : lineToInsert)
      res[i - 1].push_back(stol(num));
  }
  return res;
}
...
long Location(long location, vector<vector<long>> destionationSourceRange) {
  for (vector<long> line : destionationSourceRange) {
    ...
    if (location >= source && location < source + range)
      return location - source + destionation;
  }
  return location;
}

long reciproque(long breaking,
                vector<vector<long>> listDestionationSourceRange) {
  for (vector<long> line : listDestionationSourceRange) {
    long destination = line[0];
    long source = line[1];
    long range = line[2];
    if (breaking >= destination && breaking < destination + range)
      return breaking - destination + source;
  }
  return breaking;
}
...
long Part1(string fileContent) {
  ...
  vector<string> seeds =
      splitString(splitString(splittedInput[0], ": ")[1], " ");

  // Parsing
  vector<vector<vector<long>>> listDestionationSourceRange;
  for (int i = 1; i < splittedInput.size(); i++)
    listDestionationSourceRange.push_back(Parse(splittedInput[i]));
...
long Part2(string fileContent) {
  ...
  for (int k = size - 2; k >= 0; k--) {
    ...
    for (long breaking : breaks[k + 1])
      breaks[k].insert(reciproque(breaking, listDestionationSourceRange[k]));
  }
...
int main() {
  ...
  cout << "The result for Part1 is  " << res1 << " : "
       << (float)duration.count() / 1000 << " miliseconds" << std::endl;
  ...
  cout << "The result for Part2 is  " << res2 << " : "
       << (float)duration.count() / 1000 << " miliseconds" << std::endl;
